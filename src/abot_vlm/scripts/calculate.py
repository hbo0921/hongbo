...
def top_view_shot(image_msg):
    ...
    # 从参数服务器获取im_flag的值
    im_flag = rospy.get_param('/im_flag', 1)
    
    if im_flag == 1:
        # 保存图像
        rospy.loginfo('保存至temp/vl_now.jpg')
        cv2.imwrite('/home/abot/310117/src/abot_vlm/temp/vl_now.jpg', img_bgr)
        # 将im_flag重置为1
        rospy.set_param('/im_flag', 255)#原为1（5点45）
        # # 屏幕上展示图像
...
def yi_vision_api(PROMPT='图片中有一个计算式，请计算一下结果并输出，例如：图中内容为1+1=，你输出为2。图中的内容为2+2=，你输出4。注意，你只输出结果，比如数字2,即最后的输出一定是一个数字，除了数字一定不要展示其他内容,我只要输出的数字格式为单个字符 例如8，在终端输出的格式为“最终的数字”', img_path='/home/abot/310117/src/abot_vlm/temp/vl_now.jpg'):
    '''
    ...
    if not os.path.exists(img_path):
        rospy.logerr(f'图片路径不存在: {img_path}')
...
def main():
    ...
    rospy.loginfo('准备识别...')
    # 从参数服务器获取im_flag的值
    im_flag = rospy.set_param('/im_flag', 1)#5点43更改原值为1
