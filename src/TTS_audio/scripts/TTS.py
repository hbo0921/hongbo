...
'''
import rospy
from TTS_audio.srv import TTS,TTSResponse  # 导入ROS服务消息类型
...
# 应用配置
appid = "1018939957"  # 应用ID
token = "1WZIolO5M-LE70vDpNgEheCE3PO2TLDp"  # 访问令牌
...
def handle_tts_request(req):
    """
    处理TTS服务请求
    :param req: 服务请求对象，包含待合成文本
    :return: 服务响应对象，包含保存的音频文件路径
    """
    rospy.loginfo(f"收到TTS请求: {req.text}")  # 打印日志
    loop = asyncio.new_event_loop()  # 创建新的事件循环
    asyncio.set_event_loop(loop)  # 设置为当前线程的事件循环
    audio_file_path = loop.run_until_complete(send_tts_request(req.text))  # 发送请求并获取音频文件路径
    os.system('mplayer %s' % audio_file_path)  # 播放音频文件
    loop.close()  # 关闭事件循环
    return TTSResponse(audio_file_path)  # 返回响应

def tts_service_node():
    """
    TTS服务节点
    """
    rospy.init_node('tts_service_node')  # 初始化ROS节点
    s = rospy.Service('tts', TTS, handle_tts_request)  # 创建服务
    rospy.loginfo("TTS服务已就绪。")  # 打印日志
    rospy.spin()  # 保持节点运行

if __name__ == '__main__':
    tts_service_node()  # 启动TTS服务节点
